# ============================
# ROAD ASSESSMENT - INDIVIDUAL DEFECT REPORTS v7.0 FINAL
#
# ASSUMPTIONS:
#   1. Folder order in config = road order (start → end)
#   2. Videos within each folder (alphabetical) = road order
#   3. Road can curve/loop - GPS coordinates can increase/decrease
#   4. NO GPS SORTING - trust filesystem order completely
#
# CHAINAGE CALCULATION:
#   - Each video length = haversine(SRT_start_GPS, SRT_end_GPS)
#   - Video offset = cumulative sum of all previous video lengths
#   - Detection global_chainage = video_offset + chainage_avg_m (from JSON)
#   - Result: Continuous Ch. 0.000 → Ch. 115+ km
#
# SRT USAGE:
#   - ONLY for calculating each video's internal length
#   - NOT for sorting videos
#   - NOT for gaps between videos
#
# FEATURES:
...
# Chainage folders IN ROAD ORDER (start → end)
# Order matters! First folder = road start, last folder = road end
CHAINAGE_FOLDERS  = [
    "0.00 to 30.700",
    "Ch.30.600 to 71.800",
    "Ch. 70.00 to 115.00",
...
# SRT files location
SRT_BASE_FOLDER   = "/mnt/gdrive/chattishgrah_road_defects_everything_ex/MorTH_Dronelab/01_Videography"

# Output reports
OUTPUT_REPORT_DIR = "/mnt/gdrive/chattishgrah_road_defects_everything_ex/All_Chattisgarh_Output/Reports_Final_v7"
...
COMPRESS_THREADS   = 16
LOCAL_CACHE_DIR    = "/tmp/road_report_cache_v7"

# ── Verification ─────────────────────────────────────────────────────────────
SHOW_VERIFICATION = True  # Set False to skip verification output
...
def parse_srt_gps(srt_path):
    """Parse DJI SRT → (start_lat, start_lon, end_lat, end_lon) or None."""
    ...
    try:
        ...
        lats = re.findall(r'\[latitude\s*:\s*([-\d.]+)\]', content)
        lons = re.findall(r'\[longitude\s*:\s*([-\d.]+)\]', content)

        if not lats or not lons or len(lats) != len(lons):
            return None

        first_lat, first_lon = float(lats[0]),  float(lons[0])
        last_lat,  last_lon  = float(lats[-1]), float(lons[-1])

        for lat, lon in [(first_lat, first_lon), (last_lat, last_lon)]:
            if not (-90 <= lat <= 90 and -180 <= lon <= 180):
                return None

        return (first_lat, first_lon, last_lat, last_lon)

    except Exception:
        return None


def find_srt_file(video_name, chainage_folder_name):
    """Find SRT file for video."""
    base = os.path.splitext(video_name)[0]

    # Direct path
    for ext in ['.SRT', '.srt']:
        path = os.path.join(SRT_BASE_FOLDER, chainage_folder_name, base + ext)
    ...
    for ext in ['.SRT', '.srt']:
        matches = glob.glob(
            os.path.join(SRT_BASE_FOLDER, '**', base + ext),
            recursive=True
        )
        if matches:
            return matches[0]
...
# ============================
# LOAD VIDEOS IN FILESYSTEM ORDER
# ============================

def load_videos_filesystem_order(chainage_folders):
    """
    Load videos in EXACT filesystem order.
    NO SORTING - trust folder order + alphabetical video order.
    """
    print(f"\n📂 Loading videos in FILESYSTEM order...")
    print(f"   📋 Folder sequence (road order):")
    for i, f in enumerate(chainage_folders, 1):
        print(f"      {i}. '{f}'")
    ...
    for folder_name in chainage_folders:
        ...
        # Get video folders in alphabetical order
        video_dirs = sorted([
            d for d in os.listdir(folder_path)
            if os.path.isdir(os.path.join(folder_path, d))
        ])

        print(f"\n   📁 '{folder_name}': {len(video_dirs)} videos")
...
# ============================
# SRT METADATA (LENGTH ONLY)
# ============================

def add_srt_length(all_videos):
    """
    Read SRT for each video → calculate video length ONLY.
    NO sorting, NO gap calculation.
    """
    print(f"\n📡 Reading SRT files (for video length only)...")

    for i, video in enumerate(all_videos):
        video_name  = video['video_name']
        folder_name = video['folder_name']
        srt_name    = video['json_data'].get('srt_name', '') or video_name

        srt_path = find_srt_file(srt_name, folder_name)
        gps_data = parse_srt_gps(srt_path) if srt_path else None

        if gps_data:
            s_lat, s_lon, e_lat, e_lon = gps_data
            # Video length = distance from SRT start to SRT end
            length_m = haversine(s_lat, s_lon, e_lat, e_lon)
            
            video['start_gps'] = (s_lat, s_lon)
            video['end_gps']   = (e_lat, e_lon)
            video['length_m']  = length_m
            video['srt_ok']    = True
            
            if (i + 1) % 10 == 0 or i < 5:
                print(f"   ✅ {video_name:40s} len={length_m/1000:.3f}km")
        else:
            video['length_m'] = 0.0
            video['srt_ok']   = False
            print(f"   ❌ {video_name}: SRT not found/parsed")

    ok  = sum(1 for v in all_videos if v.get('srt_ok'))
    bad = sum(1 for v in all_videos if not v.get('srt_ok'))
    print(f"\n   ✅ SRT OK    : {ok}")
    print(f"   ❌ SRT fail  : {bad}")

    return all_videos

# ============================
# ASSIGN GLOBAL OFFSETS (CUMULATIVE)
# ============================

def assign_global_offsets(all_videos):
    """
    Assign global chainage offset using filesystem order.
    
    Cumulative formula:
      video_offset = sum of all previous video lengths
    """
    print(f"\n📏 Assigning global chainage offsets (filesystem order)...")
    print(f"   Method: Cumulative SRT video lengths")
    ...
    for i, v in enumerate(all_videos):
        vname = v['video_name']
        offsets[vname] = cumulative

        if v.get('srt_ok'):
            length_m = v['length_m']
            end_ch   = cumulative + length_m
            
            if i < 5 or (i + 1) % 10 == 0:
                print(f"   {i+1:3d}. {vname:40s} "
                      f"Ch. {cumulative/1000:.3f}km → {end_ch/1000:.3f}km "
                      f"(+{length_m/1000:.3f}km)")
            
            cumulative += length_m
        else:
            if i < 5 or (i + 1) % 10 == 0:
                print(f"   {i+1:3d}. {vname:40s} "
                      f"Ch. {cumulative/1000:.3f}km (no SRT)")

    total_km = cumulative / 1000.0
    print(f"\n   ✅ Total road distance: {total_km:.3f} km")
...
def print_verification_summary(all_videos, total_road_m):
    """Print summary verification info."""
    ...
    # Folder chainage ranges
    print(f"\n📏 Chainage ranges by folder:")
    ...
    for v in all_videos:
        fname = v['folder_name']
        if fname not in folder_ranges:
            folder_ranges[fname] = {'start': cumulative, 'videos': 0}
        
        if v.get('srt_ok'):
            cumulative += v['length_m']
        
        folder_ranges[fname]['end'] = cumulative
        folder_ranges[fname]['videos'] += 1

    for fname, data in folder_ranges.items():
        start_km = data['start'] / 1000.0
        end_km   = data['end'] / 1000.0
        print(f"   📁 '{fname}'")
        print(f"      Videos  : {data['videos']}")
        print(f"      Chainage: Ch. {start_km:.3f} → {end_km:.3f} km")

    # Total stats
    print(f"\n📊 Total Statistics:")
    print(f"   Total videos       : {len(all_videos)}")
    print(f"   Videos with SRT    : {sum(1 for v in all_videos if v.get('srt_ok'))}")
    print(f"   Total road distance: {total_road_m/1000:.3f} km")
    print(f"   Chainage range     : Ch. 0.000 → Ch. {total_road_m/1000:.3f} km")

    print(f"\n🎯 Chainage Method:")
    print(f"   ✅ Filesystem order (folders + alphabetical videos)")
    print(f"   ✅ SRT GPS used ONLY for video length calculation")
    print(f"   ✅ Cumulative continuous chainage")
    print(f"   ✅ NO GPS sorting (road can curve/loop)")

    print(f"\n{'='*80}\n")

# ============================
# BUILD GLOBAL DETECTIONS
# ============================

def build_global_detections(all_videos, offsets):
    """Build detections with global chainage."""
    print(f"\n🔗 Building global detections...")
    ...
    for v in all_videos:
        vname      = v['video_name']
        offset     = offsets.get(vname, 0.0)
        json_data  = v['json_data']
        video_dir  = os.path.dirname(v['json_path'])
        folder     = v['folder_name']

        for det in json_data.get('detections', []):
            local_ch = det.get('chainage_avg_m', 0.0)
            
            # Global chainage = video offset + local position within video
            det['global_chainage_m'] = offset + local_ch
    ...
    # Sort by global chainage (should already be in order, but ensure it)
    all_detections.sort(key=lambda x: x['global_chainage_m'])

    print(f"   ✅ Total detections: {total_raw}")
    print(f"   ✅ Sorted by global chainage")
...
def generate_individual_report(dtype, detections, total_road_m,
    ...
    doc.add_paragraph(
        f'• Sections             : {", ".join(chainage_folders)}'
    )
    doc.add_paragraph(
        f'• Chainage Method      : Filesystem order + SRT video lengths'
    )
    doc.add_paragraph(
...
def main():
    print("\n" + "="*80)
    print("ROAD ASSESSMENT — INDIVIDUAL DEFECT REPORTS v7.0 FINAL")
    print("  ✅ Filesystem order (folders + alphabetical videos)")
    print("  ✅ SRT GPS used ONLY for video length")
    print("  ✅ Cumulative continuous chainage Ch. 0 → 115+ km")
    print("  ✅ NO GPS sorting (road can curve/loop)")
    ...
    # Step 3: Load videos in filesystem order
    all_videos = load_videos_filesystem_order(CHAINAGE_FOLDERS)
    if not all_videos:
        print("❌ No videos found!")
        return

    # Step 4: Add SRT length to each video
    all_videos = add_srt_length(all_videos)

    # Step 5: Assign global offsets (cumulative)
    offsets, total_road_m = assign_global_offsets(all_videos)

    # Step 6: Verification summary
    print_verification_summary(all_videos, total_road_m)

    # Step 7: Build detections with global chainage
    all_detections, total_raw = build_global_detections(
        all_videos, offsets
    )

    print(f"\n⏱️  Pre-processing: {(time.time()-t0)/60:.1f} min")

    # Step 8: Remove duplicates (GPU)
    all_detections = remove_duplicates_gpu(all_detections)

    # Step 9: Group by defect type
